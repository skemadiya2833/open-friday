...
from friday.config import CLICK_MARKER_ENABLED

from friday.ui.capture import (
...
def _register_class() -> None:
    ...
    if _class_registered:
        return
    from friday.ui.capture import WNDCLASSEX, _DEFAULT_WNDPROC
...
def hide_aim_cursor(wait: bool = False, timeout: float = 0.8) -> None:
    ctrl = _get_controller()
    ctrl.hide()
    if wait:
        # Wait until the Win32 layered window reports hidden.
        deadline = time.monotonic() + timeout
        while time.monotonic() < deadline:
            if not ctrl._visible.is_set():
                return
            time.sleep(0.02)
...
def dismiss_click_target(wait: bool = False, **_) -> None:
    hide_aim_cursor(wait=wait)
